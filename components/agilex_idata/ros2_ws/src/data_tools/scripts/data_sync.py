...
class Operator:
    ...
    def init_directories(self):
        ...
        self.robot_base_state_dirs = [os.path.join(self.episode_dir, "robotBase/state", name) for name in self.args.robotBaseStateNames]
        self.robot_base_action_dirs = [os.path.join(self.episode_dir, "robotBase/action", name) for name in self.args.robotBaseActionNames]
    ...
    def init_time_series(self):
        ...
        self.robot_base_state_data_time_series = [[] for _ in self.args.robotBaseStateNames]
        self.robot_base_action_data_time_series = [[] for _ in self.args.robotBaseActionNames]
        ...
        self.robot_base_state_sync_time_series = [[] for _ in self.args.robotBaseStateNames]
        self.robot_base_action_sync_time_series = [[] for _ in self.args.robotBaseActionNames]
        ...
        self.robot_base_state_exts = [".json"] * len(self.args.robotBaseStateNames)
        self.robot_base_action_exts = [".json"] * len(self.args.robotBaseActionNames)
    ...
    def load_all_time_series(self):
        ...
        for i, name in enumerate(self.args.robotBaseStateNames):
            count = self.get_files_in_path(self.robot_base_state_dirs[i], ".json",
                                         self.robot_base_state_data_time_series[i],
                                         self.robot_base_state_sync_time_series[i])

        for i, name in enumerate(self.args.robotBaseActionNames):
            count = self.get_files_in_path(self.robot_base_action_dirs[i], ".json",
                                         self.robot_base_action_data_time_series[i],
                                         self.robot_base_action_sync_time_series[i])
    ...
    def check_data_adequacy(self, print_info: bool = False) -> Optional[float]:
        ...
        for i, name in enumerate(self.args.robotBaseStateNames):
            if len(self.robot_base_state_data_time_series[i]) == 0:
                if print_info:
                    print(f"Robot base state {name} has no data")
                result = False
            else:
                time = max(time, self.robot_base_state_data_time_series[i][-1].time)

        for i, name in enumerate(self.args.robotBaseActionNames):
            if len(self.robot_base_action_data_time_series[i]) == 0:
                if print_info:
                    print(f"Robot base action {name} has no data")
                result = False
            else:
                time = max(time, self.robot_base_action_data_time_series[i][-1].time)
        ...
        for time_series in self.all_time_series:
            ...
            if frame_time is not None:
                ...
                for i, name in enumerate(self.args.robotBaseStateNames):
                    if not time_diff_pass:
                        break
                    if self.robot_base_state_data_time_series[i]:
                        closest_idx, closest_diff = self.find_closest_index(
                            self.robot_base_state_data_time_series[i], frame_time)
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                        closest_indices[f'robot_base_state_{i}'] = closest_idx

                # 检查机器人底盘命令数据
                for i, name in enumerate(self.args.robotBaseActionNames):
                    if not time_diff_pass:
                        break
                    if self.robot_base_action_data_time_series[i]:
                        closest_idx, closest_diff = self.find_closest_index(
                            self.robot_base_action_data_time_series[i], frame_time)
                        if closest_diff > self.args.timeDiffLimit:
                            time_diff_pass = False
                            break
                        closest_indices[f'robot_base_action_{i}'] = closest_idx
                ...
                if time_diff_pass:
                    ...
                    # 添加机器人底盘实时状态数据到同步列表
                    for i, name in enumerate(self.args.robotBaseStateNames):
                        if f'robot_base_state_{i}' in closest_indices:
                            idx = closest_indices[f'robot_base_state_{i}']
                            self.robot_base_state_data_time_series[i][idx].to_sync_list()
                            del self.robot_base_state_data_time_series[i][:idx+1]

                    # 添加机器人底盘命令数据到同步列表
                    for i, name in enumerate(self.args.robotBaseActionNames):
                        if f'robot_base_action_{i}' in closest_indices:
                            idx = closest_indices[f'robot_base_action_{i}']
                            self.robot_base_action_data_time_series[i][idx].to_sync_list()
                            del self.robot_base_action_data_time_series[i][:idx+1]
    ...
    def write_sync_files(self):
        ...
        # 写入机器人底盘实时状态同步文件
        for i, name in enumerate(self.args.robotBaseStateNames):
            sync_file_path = os.path.join(self.robot_base_state_dirs[i], "sync.txt")
            os.makedirs(os.path.dirname(sync_file_path), exist_ok=True)
            with open(sync_file_path, 'w') as f:
                for time_series in self.robot_base_state_sync_time_series[i]:
                    f.write(f"{time_series.time:.6f}{self.robot_base_state_exts[i]}\n")

        # 写入机器人底盘命令同步文件
        for i, name in enumerate(self.args.robotBaseActionNames):
            sync_file_path = os.path.join(self.robot_base_action_dirs[i], "sync.txt")
            os.makedirs(os.path.dirname(sync_file_path), exist_ok=True)
            with open(sync_file_path, 'w') as f:
                for time_series in self.robot_base_action_sync_time_series[i]:
                    f.write(f"{time_series.time:.6f}{self.robot_base_action_exts[i]}\n")
