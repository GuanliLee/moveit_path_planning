...
class EpisodicDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(dataset_path, 'r') as root:
            ...
            # cv2.imread(os.path.join(os.path.dirname(dataset_path), root[f'/camera/color/left'][start_index].decode('utf-8')), cv2.IMREAD_UNCHANGED)
            # cv2.imread(os.path.join(os.path.dirname(dataset_path), root[f'/camera/depth/left'][start_index].decode('utf-8')), cv2.IMREAD_UNCHANGED)
            # pcl.load(os.path.join(os.path.dirname(dataset_path), root[f'/camera/pointCloud/left'][start_index].decode('utf-8'))).to_array()
