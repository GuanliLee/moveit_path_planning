...
from capture_health import build_capture_health

SUB_COMPRESSED_CAMERA = True
MIN_CAPTURE_COVERAGE_RATIO = 0.8
...
SENSOR_TOPIC_TYPE_DICT = {
    ...
    "imu" : Imu,
    "robotBase" : Odometry,
    "robotBase_state" : PoseStamped,
    "robotBase_vel" : Odometry,
    "robotBase_action" : String,
    "action_lifting" : String,
...
if LiftMotorMsg is not None:
    SENSOR_TOPIC_TYPE_DICT["lift"] = LiftMotorMsg
    SENSOR_TOPIC_TYPE_DICT["lift_motor"] = LiftMotorMsg
    TYPE_NAME_DICT[LiftMotorMsg] = "lifting_msg_pkg/msg/LiftMotorMsg"
...
class RecordMcapNode(Node):
    ...
    def topic_callback(self, msg, topic_name):
        ...
        arrival_timestamp = cur_stamp.sec + cur_stamp.nanosec / 1e9
        ...
        # Gap health always uses recorder callback time, independent of the
        # optional timestamp selected for the MCAP record itself.
        self.topic_count_dict[topic_name].add_count_and_time(arrival_timestamp)
    ...
    def start_record_mcap(self):
        ...
        self.capture_wall_start = time.monotonic()
        self.capture_wall_end = self.capture_wall_start
        self.rate_failed_topics = set()
    ...
    def start_sub_and_thread(self):
        ...
        for sensor_type, topic_list in self.sensor_topic_dict.items():
            if sensor_type not in SENSOR_TOPIC_TYPE_DICT:
                raise RuntimeError(
                    f"Unsupported sensor type '{sensor_type}' from {self.topic_config}. "
                    "Check ROS message dependencies and SENSOR_TOPIC_TYPE_DICT."
                )
            for i in range(len(topic_list)):
                mcap_topic_name = sub_topic_name = topic_list[i]
                mcap_topic_type = sub_topic_type = SENSOR_TOPIC_TYPE_DICT[sensor_type]
                if SUB_COMPRESSED_CAMERA and sensor_type in ("camera", "camera_color", "camera_depth"):
    ...
    def stop_record_mcap(self):
        self.thread_run = False
        self.capture_wall_end = time.monotonic()
        ...
        self.write_capture_health()
        path = os.path.join(self.mcap_path, f"episode{self.episode_index}_0.mcap")
        print(f"---\n{path} info:\n")
        print(run_shell_command(f"mcap info {path}"))

    def write_capture_health(self):
        duration = max(self.capture_wall_end - self.capture_wall_start, 0.0)
        health = build_capture_health(
            topic_counts={topic: counter.get_count() for topic, counter in self.topic_count_dict.items()},
            topic_min_rates=self.topic_hz_dict,
            duration_seconds=duration,
            rate_failed_topics=self.rate_failed_topics,
            min_coverage_ratio=MIN_CAPTURE_COVERAGE_RATIO,
            topic_max_gaps_seconds={
                topic: counter.get_max_gap_seconds()
                for topic, counter in self.topic_count_dict.items()
            },
            max_camera_gap_seconds=MAX_CAMERA_GAP_SECONDS,
        )
        health["episode_index"] = self.episode_index
        health_path = os.path.join(self.mcap_path, "capture_health.json")
        tmp_path = f"{health_path}.tmp"
        with open(tmp_path, "w", encoding="utf-8") as handle:
            json.dump(health, handle, ensure_ascii=False, indent=2)
            handle.write("\n")
        os.replace(tmp_path, health_path)
        print(f"capture health: {health_path} camera_ok={health['camera_ok']}")
            ...
            for topic_name in self.topic_count_dict.keys():
                ...
                if count_in_sec < check_hz or cur_frequency < check_hz:
                    if (cur_time_sec - topic_last_up_time_dict[topic_name] > self.timeout):
                        ...
                        msg.fail = True
                        self.rate_failed_topics.add(topic_name)
                    else:
