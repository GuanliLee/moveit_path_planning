...
public:
    ...
    DataCapture(std::string name, const rclcpp::NodeOptions & options, std::string datasetDir, int episodeIndex, int hz, int timeout, double cropTime=-1, bool useService=false): DataUtility(name, options, datasetDir, episodeIndex) {
        ...
        subCameraColorConfigs = std::vector<rclcpp::Subscription<sensor_msgs::msg::CameraInfo>::SharedPtr>(cameraColorConfigTopics.size());
        ...
        cameraColorConfigMsgCounts = std::vector<int>(cameraColorConfigTopics.size(), 0);
        ...
        cameraColorConfigMsgCountMtxs = std::vector<std::mutex>(cameraColorConfigTopics.size());
        ...
        for(int i = 0; i < cameraColorConfigTopics.size(); i++){
