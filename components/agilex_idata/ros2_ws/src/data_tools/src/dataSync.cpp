...
public:
    ...
    std::vector<std::vector<TimeSeries>> robotBaseStateDataTimeSeries;
    std::vector<std::vector<TimeSeries>> robotBaseActionDataTimeSeries;
    ...
    std::vector<std::vector<TimeSeries>> robotBaseStateSyncTimeSeries;
    std::vector<std::vector<TimeSeries>> robotBaseActionSyncTimeSeries;
    ...
    std::vector<std::string> robotBaseStateExts;
    std::vector<std::string> robotBaseActionExts;
    ...
    DataSync(std::string name, const rclcpp::NodeOptions & options, std::string datasetDir, int episodeIndex, double timeDiffLimit): DataUtility(name, options, datasetDir, episodeIndex) {
        ...
        robotBaseStateDataTimeSeries = std::vector<std::vector<TimeSeries>>(robotBaseStateNames.size());
        robotBaseActionDataTimeSeries = std::vector<std::vector<TimeSeries>>(robotBaseActionNames.size());
        ...
        robotBaseStateSyncTimeSeries = std::vector<std::vector<TimeSeries>>(robotBaseStateNames.size());
        robotBaseActionSyncTimeSeries = std::vector<std::vector<TimeSeries>>(robotBaseActionNames.size());
        ...
        robotBaseStateExts = std::vector<std::string>(robotBaseStateNames.size());
        robotBaseActionExts = std::vector<std::string>(robotBaseActionNames.size());
        ...
        for(int i = 0; i < robotBaseStateNames.size(); i++){
            getFileInPath(robotBaseStateDirs.at(i), ".json", &robotBaseStateDataTimeSeries.at(i), &robotBaseStateSyncTimeSeries.at(i));
            robotBaseStateExts.at(i) = ".json";
        }
        for(int i = 0; i < robotBaseActionNames.size(); i++){
            getFileInPath(robotBaseActionDirs.at(i), ".json", &robotBaseActionDataTimeSeries.at(i), &robotBaseActionSyncTimeSeries.at(i));
            robotBaseActionExts.at(i) = ".json";
        }
    ...
    int findClosestIndex(const std::vector<TimeSeries>& dataTimeSeries, double frameTime){
        int closerIndex = 0;
        double closerTimeDiff = INFINITY;
        for(int j = 0; j < dataTimeSeries.size(); j++){
            double timeDiff = fabs(dataTimeSeries.at(j).time - frameTime);
    ...
    void sync(){
        ...
        for(int i = 0; i < allTimeSeries.size(); i++){
            ...
            if(frameTime != INFINITY){
                ...
                std::vector<double> robotBaseStateCloserIndexs = std::vector<double>(robotBaseStateNames.size(), 0);
                std::vector<double> robotBaseActionCloserIndexs = std::vector<double>(robotBaseActionNames.size(), 0);
                ...
                for(int i = 0; i < robotBaseStateNames.size() && timeDiffPass; i++){
                    if(!robotBaseStateToSyncs.at(i))
                        continue;
                    int closerIndex = 0;
                    double closerTimeDiff = INFINITY;
                    for(int j = 0; j < robotBaseStateDataTimeSeries.at(i).size(); j++){
                        double timeDiff = fabs(robotBaseStateDataTimeSeries.at(i).at(j).time - frameTime);
                        if(timeDiff < closerTimeDiff){
                            closerTimeDiff = timeDiff;
                            closerIndex = j;
                        }
                    ...
                    if(closerTimeDiff > timeDiffLimit)
                        timeDiffPass = false;
                    robotBaseStateCloserIndexs.at(i) = closerIndex;
                }
                for(int i = 0; i < robotBaseActionNames.size() && timeDiffPass; i++){
                    if(!robotBaseActionToSyncs.at(i))
                        continue;
                    int closerIndex = 0;
                    double closerTimeDiff = INFINITY;
                    for(int j = 0; j < robotBaseActionDataTimeSeries.at(i).size(); j++){
                        double timeDiff = fabs(robotBaseActionDataTimeSeries.at(i).at(j).time - frameTime);
                        if(timeDiff < closerTimeDiff){
                            closerTimeDiff = timeDiff;
                            closerIndex = j;
                        }
                    ...
                    if(closerTimeDiff > timeDiffLimit)
                        timeDiffPass = false;
                    robotBaseActionCloserIndexs.at(i) = closerIndex;
                }
                ...
                for(int i = 0; i < robotBaseStateNames.size(); i++){
                    if(robotBaseStateToSyncs.at(i)){
                        robotBaseStateDataTimeSeries.at(i).at(robotBaseStateCloserIndexs.at(i)).toSyncList();
                        robotBaseStateDataTimeSeries.at(i).erase(robotBaseStateDataTimeSeries.at(i).begin(), robotBaseStateDataTimeSeries.at(i).begin() + robotBaseStateCloserIndexs.at(i) + 1);
                    }else if(!robotBaseStateDataTimeSeries.at(i).empty()){
                        // Soft-sample low-rate base localization onto the main timeline without gating frame creation.
                        int closerIndex = findClosestIndex(robotBaseStateDataTimeSeries.at(i), frameTime);
                        robotBaseStateDataTimeSeries.at(i).at(closerIndex).toSyncList();
                    }
                }
                for(int i = 0; i < robotBaseActionNames.size(); i++){
                    if(!robotBaseActionToSyncs.at(i))
                        continue;
                    robotBaseActionDataTimeSeries.at(i).at(robotBaseActionCloserIndexs.at(i)).toSyncList();
                    robotBaseActionDataTimeSeries.at(i).erase(robotBaseActionDataTimeSeries.at(i).begin(), robotBaseActionDataTimeSeries.at(i).begin() + robotBaseActionCloserIndexs.at(i) + 1);
                }
                for(int i = 0; i < liftMotorNames.size(); i++){
                    if(!liftMotorToSyncs.at(i))
                        continue;
        ...
        for(int i = 0; i < robotBaseStateNames.size(); i++){
            std::ofstream file(robotBaseStateDirs.at(i) + "/sync.txt");
            for(int j = 0; j < robotBaseStateSyncTimeSeries.at(i).size(); j++)
                file<<std::to_string(robotBaseStateSyncTimeSeries.at(i).at(j).time)<<robotBaseStateExts.at(i)<<std::endl;
            file.close();
        }
        for(int i = 0; i < robotBaseActionNames.size(); i++){
            std::ofstream file(robotBaseActionDirs.at(i) + "/sync.txt");
            for(int j = 0; j < robotBaseActionSyncTimeSeries.at(i).size(); j++)
                file<<std::to_string(robotBaseActionSyncTimeSeries.at(i).at(j).time)<<robotBaseActionExts.at(i)<<std::endl;
            file.close();
        }
        for(int i = 0; i < liftMotorNames.size(); i++){
    ...
    double checkDataAdequacy(bool print=false){
        ...
        for(int i = 0; i < robotBaseStateNames.size() && result; i++){
            if(robotBaseStateToSyncs.at(i)){
                if(robotBaseStateDataTimeSeries.at(i).size() == 0){
                    if(print){
                        std::cout<<"robotBase state "<<robotBaseStateNames.at(i)<<" has no data"<<std::endl;
                    }
                    result = false;
                }
                else
                    time = time > robotBaseStateDataTimeSeries.at(i).back().time ? time : robotBaseStateDataTimeSeries.at(i).back().time;
            }
        }
        for(int i = 0; i < robotBaseActionNames.size() && result; i++){
            if(robotBaseActionToSyncs.at(i)){
                if(robotBaseActionDataTimeSeries.at(i).size() == 0){
                    if(print){
                        std::cout<<"robotBase action "<<robotBaseActionNames.at(i)<<" has no data"<<std::endl;
                    }
                    result = false;
                }
                else
                    time = time > robotBaseActionDataTimeSeries.at(i).back().time ? time : robotBaseActionDataTimeSeries.at(i).back().time;
            }
        }
