#!/usr/bin/env python3
# -*-coding:utf8-*-
# This file controls a single robotic arm node and handles the movement of
# the robotic arm with a gripper.
import math
import threading
import time
...
STATE_PUBLISH_RATE_HZ = 180


class PiperRosNode(Node):
    """Provide the ROS 2 node for the robotic arm."""
    ...
    def __init__(self) -> None:
        ...
        self.gripper_val_mutiple = self.get_parameter(
            'gripper_val_mutiple').get_parameter_value().integer_value
        ...
        self.joint_states_feedback.name = ['joint1', 'joint2',
    ...
    def publish_thread(self):
        ...
        rate = self.create_rate(STATE_PUBLISH_RATE_HZ)
        enable_flag = False
        # Set timeout (seconds)
        timeout = 5
            ...
            if (self.auto_enable):
                while not (enable_flag):
                    ...
                    enable_flag = (
                        ...
                        and self.piper.GetArmLowSpdInfoMsgs()
                    ...
                    gripper_feedback_ready = True
                    if self.gripper_exist:
                        gripper_messages = self.piper.GetArmGripperMsgs()
                        gripper_feedback_ready = gripper_messages.time_stamp > 0
                        if gripper_feedback_ready:
                            current_gripper_angle = (
                                gripper_messages.gripper_state.grippers_angle
                            )
                            self.piper.GripperCtrl(
                                current_gripper_angle, 1000, 0x01, 0
                            )
                    enable_flag = enable_flag and gripper_feedback_ready
    ...
    def PublishArmState(self):
        ...
        arm_status.communication_status_joint_1 = self.piper.GetArmStatus(
        ).arm_status.err_status.communication_status_joint_1
        arm_status.communication_status_joint_2 = self.piper.GetArmStatus(
        ).arm_status.err_status.communication_status_joint_2
        arm_status.communication_status_joint_3 = self.piper.GetArmStatus(
        ).arm_status.err_status.communication_status_joint_3
        arm_status.communication_status_joint_4 = self.piper.GetArmStatus(
        ).arm_status.err_status.communication_status_joint_4
        arm_status.communication_status_joint_5 = self.piper.GetArmStatus(
        ).arm_status.err_status.communication_status_joint_5
        arm_status.communication_status_joint_6 = self.piper.GetArmStatus(
    ...
    def PublishArmJointAndGripper(self):
        ...
        new_time = max(
            self.piper.GetArmJointMsgs().time_stamp,
            self.piper.GetArmHighSpdInfoMsgs().time_stamp)
        self.joint_states.header.stamp = self.float_to_ros_time(new_time)
        # Here, you can set the joint positions to any value you want
        # The raw data obtained is in degrees multiplied by 1000. To convert to
        # radians, divide by 1000, multiply by π/180, and limit to 5 decimal
        # places
        ...
        # Piper reports the total finger opening. URDF joint7 represents one
        # finger, while joint8 mimics it in the opposite direction.
        joint_6: float = self.piper.GetArmGripperMsgs().gripper_state.grippers_angle / 2000000
        ...
        self.joint_states.position = [joint_0, joint_1,
        ...
        self.joint_states_feedback.position = [
            joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, joint_6]
        self.joint_states_feedback.velocity = [vel_0, vel_1, vel_2, vel_3, vel_4, vel_5, 0.0]
        self.joint_states_feedback.effort = [
            effort_0,
            effort_1,
            effort_2,
            effort_3,
    ...
    def PublishArmCtrlAndGripper(self):
        ...
        new_time = max(self.piper.GetArmJointCtrl().time_stamp,
                       self.piper.GetArmGripperCtrl().time_stamp)
        ...
        joint_6: float = self.piper.GetArmGripperCtrl().gripper_ctrl.grippers_angle / 2000000
        self.joint_ctrl.position = [joint_0, joint_1, joint_2,
                                    joint_3, joint_4, joint_5, joint_6]  # Example values
    ...
    def pos_callback(self, pos_data):
        """Process an end-effector pose command."""
        ...
        if self.GetEnableFlag():
            ...
            if self.gripper_exist:
                if len(joint_data.effort) >= 7:
                    gripper_effort = clip(joint_data.effort[6], 0.5, 5)
    ...
    def handle_enable_service(self, req, resp):
        """Handle an arm enable service request."""
